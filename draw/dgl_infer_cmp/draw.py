...
# 读取所有数据
gcn = read_and_merge('gnna_gcn.csv', 'dgl_gcn.csv')
gin = read_and_merge('gnna_gin.csv', 'dgl_gin.csv')
...
for i, (bar_gcn, bar_gin) in enumerate(zip(bars_gcn, bars_gin)):
    if bar_gcn.get_height() > y_max:
        plt.text(bar_gcn.get_x() + bar_gcn.get_width() / 2, y_max - 0.2,
                 f'{bar_gcn.get_height():.1f}×', ha='center', va='top', fontsize=14, fontweight='bold', rotation=90, color='black')
    if bar_gin.get_height() > y_max:
        plt.text(bar_gin.get_x() + bar_gin.get_width() / 2, y_max - 0.2,
                 f'{bar_gin.get_height():.1f}×', ha='center', va='top', fontsize=14, fontweight='bold', rotation=90, color='black')
